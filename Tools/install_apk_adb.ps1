...
#
# Default APK: {projectRoot}/com.densappstudio.ancientegyptianmathematics.apk
# Package id:  com.densappstudio.ancient.egyptian.mathematics
#
# If install fails with signature mismatch:
#   adb uninstall com.densappstudio.ancient.egyptian.mathematics
...
if ([string]::IsNullOrWhiteSpace($ApkPath)) {
    $ApkPath = Join-Path $root "com.densappstudio.ancientegyptianmathematics.apk"
...
If signature mismatch with an older build:
  `"$adb`" -s $targetSerial uninstall com.densappstudio.ancient.egyptian.mathematics
then re-run with -SkipBuild.
"@
    exit $code
}

Write-Host "Done: installed $ApkPath on $targetSerial (package com.densappstudio.ancient.egyptian.mathematics)."
exit 0
